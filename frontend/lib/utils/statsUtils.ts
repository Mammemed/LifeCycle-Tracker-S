import type { IEntity } from '@/lib/models/Entity'

export function computeStatusDistribution(entities: IEntity[]): Record<string, number> {
...
export function computeAverageTimePerStatus(entities: IEntity[]): Record<string, number> {
...
export function computeSuccessRate(entities: IEntity[]): number {
...
export function computeStagesCountPerEntity(entities: IEntity[]) {
...
export function computeUserActivityHeatmap(entities: IEntity[]): Record<string, number> {
...
export function predictSuccessProbability(entity: IEntity, allEntities: IEntity[]) {
  const finalStates = ['approved', 'published', 'completed', 'accepted']
  ...
  const statusMultiplier = statusWeights[entity.currentStatus] || 0.5
  ...
  if (entity.statusHistory.length > 0) {
    ...
    const daysInStatus = (new Date().getTime() - lastChange.getTime()) / (1000 * 60 * 60 * 24)
    if (daysInStatus > 30) {
      timeMultiplier = 0.7
...
export function estimateTimeRemaining(entity: IEntity, allEntities: IEntity[]) {
  const finalStates = ['approved', 'published', 'completed', 'accepted']
  ...
  similarEntities.forEach(similarEntity => {
    if (similarEntity.statusHistory.length > 0) {
      const creationTime = new Date(similarEntity.createdAt).getTime()
      const completionEntry = similarEntity.statusHistory.find(sh => 
        finalStates.includes(sh.toStatus)
      )
      
      if (completionEntry) {
        const completionTime = new Date(completionEntry.changedAt).getTime()
  ...
  const creationTime = new Date(entity.createdAt).getTime()
  const timeSpent = (new Date().getTime() - creationTime) / (1000 * 60 * 60 * 24)
...
export function generatePredictions(allEntities: IEntity[]) {
  const predictions = allEntities.map(entity => {
